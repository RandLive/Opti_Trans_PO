function [X,FVAL,EXITFLAG,OUTPUT] = patternsearch_self(FUN,initialX,Aineq,Bineq,Aeq,Beq,LB,UB,nonlcon,options)
...
%
%   See also OPTIMOPTIONS, GA, PSOUTPUTFCNTEMPLATE, SEARCHFCNTEMPLATE, @.
...
msg = isoptimargdbl('PATTERNSEARCH', {'X0','A',   'b',   'Aeq', 'beq','LB','UB'}, ...
                           initialX,  Aineq, Bineq, Aeq,   Beq,  LB,  UB);
if ~isempty(msg)
    error('globaloptim:patternsearch:dataType',msg);
end
...
if isempty(FUN) ||  ~(isa(FUN,'inline') || isa(FUN,'function_handle'))
    error(message('globaloptim:patternsearch:needFunctionHandleObj'));
end
...
if ~isempty(nonlcon) && ~(isa(nonlcon,'inline') || isa(nonlcon,'function_handle'))
    error(message('globaloptim:patternsearch:needFunctionHandleConstr'));
end
...
if(~isempty(initialX))
    ...
    error(message('globaloptim:patternsearch:initialPoint'));
end
...
[Iterate.x,Aineq,Bineq,Aeq,Beq,LB,UB,msg,EXITFLAG] = ...
    preProcessLinearConstr(Iterate.x,Aineq,Bineq,Aeq,Beq,LB,UB,options.TolCon, ...
